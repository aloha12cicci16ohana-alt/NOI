...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
    ...
    const footerAnimWrapper = document.querySelector(
    ...
    filterBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        workItems.forEach((item) => {
          ...
          if (
            filterValue === "all" ||
            (itemCategories && itemCategories.includes(filterValue))
          ) {
...
window.addEventListener("load", () => {
  ...
  if (
    ...
    !sessionStorage.getItem("noi_loading_shown")
  ) {
...
(function () {
  ...
  // リンク・ボタンのホバーで拡大
  const hoverTargets =
    "a, button, .filter-btn, .view-all-btn, .cta-btn, .work-item";
